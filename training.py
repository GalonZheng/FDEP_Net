...
from network_FDEP_Net import SpatialTransformer_block
from network_FDEP_Net import FDNet
import dataloader as myloader
from validation import NJD, dice_coef
...
def train(dataset,
    ...
    if dataset == 'OASIS':
        train_dir = '/Extra/3D_brain_MRI/affine_img/'
        ...
        train_dir = '/Extra//3D_IXI_dataset/IXI_data/Train/'
        valid_dir = '/Extra//3D_IXI_dataset/IXI_data/Test/'
        ...
        print('No such dataset called:', dataset)
        exit(-1)
    ...
    if model_name == 'FDNet':
        model = FDNet()
        Losses = [losses.NCC(win=9).loss, losses.Grad().loss, losses.KL(prior_lambda=50).loss]
        Losses_weights = [1.0, 0.02, 0.01]
    ...
    for epoch in range(epochs):
        ...
        for pair_t, mi_t, fi_t, ml_t, fl_t in loader_train:
            ...
            print('\ncurrent data:{} and {}, training {}/{}, training loss {:.6f} = {:.6f} + {:.6f} + {'
                  ':.6f}'.format(pair_t[0], pair_t[1], step_t, data_train_sum, train_loss, loss_list[0],
                                 loss_list[1], loss_list[2]), end='')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_name", type=str, default='FDNet')
